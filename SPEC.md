...
- 画像ブロック検出時は `image_blocks=N`, `image_policy=replace`, `sanitized=true/false` をログに記録（画像内容・本文は記録しない）

#### 画像ブロック自動処理

**概要**: Claude Desktop の同じスレッド内で過去に画像を貼ったあと、背後のモデルを DeepSeek などの非 vision モデルに切り替えると、会話履歴内の画像ブロックが upstream に送られてエラーになる問題を解決する。

**処理順序**:
1. リクエストボディを読む
2. 要求モデル名を取得
3. `model_map` で upstream モデル名に書き換え
4. `model_capabilities` で upstream モデルの vision 対応有無を判定
5. 非 vision で画像ブロックがある場合、`non_vision_image_policy` に従って処理:
   - `"replace"`（デフォルト）: 画像ブロックをプレースホルダテキストに置換
   - `"drop"`: 画像ブロックを削除（空になる場合はプレースホルダ挿入）
   - `"reject"`: upstream へ送らず 400 エラーを返す
6. ログに画像ブロック件数などを記録（内容は記録しない）
7. upstream へ送信

**画像ブロック検出**: 以下の type を画像として認識する:
- `image`（Anthropic Messages API 標準）
- `input_image`（将来の互換性）
- `image_url`（将来の互換性）

検出は再帰的に行い、`tool_result.content` 内にネストされた画像ブロックも対象とする。

**ログ出力**: API キー・base64 画像データ・会話本文は一切ログに出さない。画像ブロック件数とポリシー名のみ記録する。

**適用エンドポイント**: `POST /v1/messages` と `POST /v1/messages/count_tokens` の両方に適用。
...
| `model_capabilities` | object | モデルごとの能力マップ（例: `{"vision": false}`）。未定義のモデルは `vision: false` 扱い |
| `non_vision_image_policy` | string | 非 vision モデルへ画像を送る際の動作。`"replace"`(デフォルト), `"drop"`, `"reject"` のいずれか |
...
- When image blocks are detected, `image_blocks=N`, `image_policy=replace`, `sanitized=true/false` are logged (no image content or conversation text)

#### Image Block Auto-Handling

**Overview**: When switching from a vision-capable model to a non-vision model (e.g., DeepSeek) within the same Claude Desktop thread, previous image blocks in the conversation history would cause errors. This feature detects and handles image blocks before forwarding to non-vision upstream models.

**Processing order**:
1. Read request body
2. Get requested model name
3. Rewrite to upstream model via `model_map`
4. Check upstream model's vision capability via `model_capabilities`
5. If non-vision and image blocks exist, apply `non_vision_image_policy`:
   ...
   - `"reject"`: Return 400 error instead of forwarding
6. Log image block count (no content logged)
7. Forward to upstream

**Image detection**: Recognizes these `type` values as images:
- `image` (standard Anthropic Messages API)
- `input_image` (future compatibility)
- `image_url` (future compatibility)

Detection is recursive and covers image blocks nested inside `tool_result.content`.

**Logging**: API keys, base64 image data, and conversation text are never logged. Only image block count and policy name are recorded.

**Applied to**: Both `POST /v1/messages` and `POST /v1/messages/count_tokens`.
...
| `model_capabilities` | object | Per-model capability map (e.g. `{"vision": false}`). Undefined models default to `vision: false` |
| `non_vision_image_policy` | string | Behavior when image blocks go to a non-vision model: `"replace"` (default), `"drop"`, or `"reject"` |
